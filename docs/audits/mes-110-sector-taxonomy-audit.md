...
| `validate_industry_sector_values()` function | DB (baseline migration) | validates values against `linkedin_industries.industry_group` + `legacy_industry_mapping.legacy_value`. **NOTE (corrected during step 5):** the `validate_intake_industry` trigger that invoked it on `user_intake_forms` was **deliberately dropped** in `20260615042500` — v2 intake intentionally accepts custom/free-text industries and the blocking check surfaced as "Generation failed". The function is retained as a named utility but is **not** an active guard. |
...
- **Option A — adopt the pinned 147 codes as the platform spine (ticket as written).** Replace the V2 hierarchy everywhere: forms present 147 options, `sector_tags` re-tagged to 147-code slugs, matcher and trigger rewritten. *Cost:* re-migrates every canonical surface that is already clean, discards the working 152-group intake UX and the group→sector rollup that powers overlap scoring, and forces awkward mappings for the five umbrella concepts above. **It also silently invalidates the shipped scorer calibrations** (PRs #308/#313, Jul 2026): `OVERTAG_THRESHOLD = 5` in `matchScoring.ts` means "claims ~25 % of the 20-sector V2 vocabulary" — under 147 codes the same 5 tags are 3 % and the over-tag demotion effectively stops firing — and `HORIZONTAL_SECTORS` names the V2 slug strings verbatim, so the horizontal-only rule goes dead; both would need an explicit recalibration pass or the recent relevance improvements quietly regress. Highest churn, highest mislabel risk, no user-visible gain over Option B.
...
4. **Tag-coverage backfill** (the §4 gaps; arguably the highest report-quality win): propose `sector_tags` for the 108 untagged events, 45 innovation orgs, 14 investors, and real tags for the 95 all-agnostic service providers — derived from the merge mapping of their existing raw values where present, AI-assisted + operator-reviewed where absent. Propose-only artefact first (consistent with the report-quality loop's review pattern). *Scorer-interaction note:* the moment providers gain real tags (and presumably drop `sector_agnostic`), they become subject to the specialist / over-tag / horizontal-only scoring rules (`matchScoring.ts`, PRs #279/#308/#313) **for the first time** — provider ranking will genuinely change, which is the point, but the verification must include a before/after provider-slate parity check on 2–3 representative intake profiles (baselines from the Jul 2026 duplication analysis: EY/KPMG appeared in 10 of 15 reports pre-backfill), and per-row tag counts should stay ≤ 4 so freshly-tagged rows don't immediately trip the over-tag demotion. This step is independent of step 3's column remaps (providers have no raw sector values to remap) and can run as soon as the §2 decision is locked.
5. **User-side intake — REVISED to matcher-only.** The original plan here was to remap `user_intake_forms.industry_sector` / `end_buyer_industries` with `_legacy` copies and re-add a canonical-validating trigger. Two facts found during implementation changed this: (a) the `validate_intake_industry` trigger was **deliberately dropped** in `20260615042500` because v2 intake intentionally supports custom/free-text industries — re-adding it reverses a documented decision and resurfaces "Generation failed"; (b) an empirical replay of all 92 distinct live intake values through the matcher (`industryGroupsToSectorSlugs`) showed **81/92 already resolve** to the correct canonical sector slug at match time, so rewriting the stored user rows yields **no matching improvement** while changing user-entered records tied to already-generated (snapshot) reports. The real gap was **8 clear sector values the matcher silently dropped** (`banking`, `Investment Banking`, `cyber`, `Apparel & Fashion`, `Restaurants`, `Commercial Real Estate`, `Law Practice`). Fix = extend `SECTOR_KEYWORD_ALIASES` (slugs aligned to `legacy_industry_mapping.linkedin_sector`) + a regression test; **no user-data remap, no re-added trigger.** The 3 remaining unresolved values (`Project Management`, `Customer Experience`, `Field Management`) are horizontal business-functions, not industries, and stay unresolved by design. Past `user_reports` are not rewritten (snapshots); `raw_input` JSON already preserves originals.
6. **Switch consumers — DELIVERED.** `useSectorContent`'s dead `sector_tags.cs.{<page slug>}` clause fixed via a new `industry_sectors.canonical_sector_slugs[]` column ("folded into the reference table"): `fintech → financial-services`, `medtech → hospitals-and-health-care`; `telecoms` left NULL on purpose (its canonical sector `technology-information-and-media` also holds all software/AI/data content — too broad; the hook drops the sector clause and keyword-matches instead). Confirmed: the fixed FinTech clause surfaces 20 published `financial-services` items the dead `{fintech}` clause returned 0 of. Dead, unimported `src/config/sectors.ts` deleted. Directory dropdown wiring remains MES-108's; master-search `lead_databases.sector` unaffected (already canonical).
7. **Verification — PASS.** No non-canonical value remains reachable in the matcher-facing `sector_tags` across all 8 content tables (service_providers / events / innovation_ecosystem / investors / content_items / community_members / trade_investment_agencies / leads → 0 off-taxonomy values). Intake resolves 89/92 live values (3 deliberate horizontals excluded). `_legacy` backup columns are retained as reversible safety nets and can be dropped in a follow-up once the operator signs off.
