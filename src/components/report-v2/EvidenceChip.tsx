...
const EvidenceChip = ({ chip, className }: EvidenceChipProps) => {
  if (chip === "sourced") {
    return (
      <span aria-label="sourced" className={cn("text-[9.5px] font-bold text-report-good", className)}>
        ●
      </span>
    );
  }
  if (chip === "est") {
    return (
      <span
        ...
        className={cn(
          "rounded [vertical-align:1px] bg-report-warn-tint px-1.5 py-0.5 text-[9.5px] font-bold text-report-warn",
          className
        )}
      >
        ◐ EST
  ...
  return (
    <span
      aria-label="inferred"
      className={cn(
        "rounded [vertical-align:1px] bg-report-inferred-bg px-1.5 py-0.5 text-[9.5px] font-semibold text-report-inferred-text",
        className
      )}
    >
      ○ INFERRED
