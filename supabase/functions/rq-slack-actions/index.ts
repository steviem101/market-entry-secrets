...
async function respond(responseUrl: string | undefined, text: string): Promise<Response> {
  if (responseUrl) {
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  const decision = actionId.startsWith("rq_accept") ? "accepted" : actionId.startsWith("rq_reject") ? "rejected" : null;
  if (!decision || !/^[0-9a-f-]{36}$/.test(proposalId)) return new Response("ok", { status: 200 });
  const ref = proposalId.slice(0, 8);
  const slackUser: string = payload.user?.username ?? payload.user?.name ?? payload.user?.id ?? "unknown";
  ...
  if (readErr) { log("read error", readErr.message); return await respond(responseUrl, `⚠️ \`[${ref}]\` lookup failed: ${readErr.message}`); }
  ...
  const { error: updErr } = await supabase.from("report_quality_proposals")
    ...
    .eq("id", proposalId);
  if (updErr) { log("update error", updErr.message); return await respond(responseUrl, `⚠️ Failed to update \`[${ref}]\`: ${updErr.message}`); }

  log("proposal reviewed", { ref, decision, by: slackUser });
  const suffix = decision === "accepted"
    ? "\n_Next: tell Claude Code to implement accepted proposals — the loop is propose-only, changes still ship via PRs._"
    : "";
  return await respond(responseUrl, `${decision === "accepted" ? "✅" : "❌"} \`[${ref}]\` *${row.title}* — *${decision}* by ${slackUser}.${suffix}`);
});
