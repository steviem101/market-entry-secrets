-- MES agent loops (MES-206, dashboard review fix): re-map demand-signal 'ack' to canonical
-- 'approved' in the agent_proposals view.
--
-- Bug (found in the /admin/agents max code review): agent-actions writes approvedValue 'ack'
-- when an admin approves a directory_demand_signals row, but the view normalised 'ack' back to
-- canonical 'pending' — so approving a demand signal was a permanent no-op loop in the dashboard
-- (success toast, row re-renders as pending forever, pending counts never drop). 'ack'
-- (acknowledged, awaiting action) is a reviewed-open state, i.e. canonical 'approved';
-- 'actioned' remains 'applied'.
--
-- Full CREATE OR REPLACE of the applied 20260720100200 definition with ONE changed CASE arm
-- (branch 4, directory_demand_signals). Grants/policies from the original migration persist and
-- are not restated. Additive + reversible.
-- Rollback: supabase/rollback/20260722090000_agent_proposals_view_ack_approved_revert.sql
...
select
  ...
  case g.status when 'new' then 'pending' when 'ack' then 'approved'
