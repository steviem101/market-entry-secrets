...
) AS v(slug, question, html, md, ord) ON v.slug = s.slug AND s.content_id = (SELECT id FROM public.content_items WHERE slug = 'netflix-streaming-australia-launch' AND content_type = 'case_study');
...
- Acquired by Square/Block for A\$39 billion (announced 2021, completed 2022)', 5),
      ('lessons-for-market-entrants', NULL::text, '<p>Afterpay demonstrates the value of dominating the home market first: Australia''s contained retail ecosystem let a two-sided network reach critical mass fast, creating the template — and the balance sheet — for international expansion.</p>', 'Afterpay demonstrates the value of dominating the home market first: Australia''s contained retail ecosystem let a two-sided network reach critical mass fast, creating the template — and the balance sheet — for international expansion.', 6)
) AS v(slug, question, html, md, ord) ON v.slug = s.slug AND s.content_id = (SELECT id FROM public.content_items WHERE slug = 'afterpay-buy-now-pay-later-revolution' AND content_type = 'case_study');
...
FROM (VALUES
      ('Background', 'background', 1),
      ('Entry strategy', 'entry-strategy', 2),
      ('Founders & local operations', 'founders-and-local-operations', 3),
      ('Success factors', 'success-factors', 4),
      ('Key metrics & performance', 'key-metrics-and-performance', 5),
      ('Lessons for market entrants', 'lessons-for-market-entrants', 6)
) AS v(title, slug, ord);
...
- **Local stock, no local office.** Fulfilment runs through Australian warehouses and third-party delivery partners, keeping the entry asset-light: inventory and logistics localised, everything else (design, engineering, marketing, support) run from Singapore.', 6),
      ('success-factors', NULL::text, '<ul>
...
- Willingness to adapt the physical product (sizing, ergonomics) per market', 7),
      ('key-metrics-and-performance', NULL::text, '<ul>
...
- Revenue around **S\$350 million**', 8),
      ('lessons-for-market-entrants', NULL::text, '<ol>
...
4. **Let demand pull you in.** Organic orders are the cheapest market-research programme available.', 9)
) AS v(slug, question, html, md, ord) ON v.slug = s.slug AND s.content_id = (SELECT id FROM public.content_items WHERE slug = 'secretlab-anz-market-entry' AND content_type = 'case_study');
...
FROM (VALUES
      ('Background', 'background', 1),
      ('Entry strategy', 'entry-strategy', 2),
      ('Launch & local iteration', 'launch-and-local-iteration', 3),
...
FROM public.content_sections s
JOIN (VALUES
      ('background', NULL::text, '<p>Founded in Singapore in August 2014 by Henry Chan, Joel Leong and co-founders, ShopBack built a simple flywheel: retailers pay it a commission for referred sales, and it shares part of that commission back with shoppers as cashback. After winning across Southeast Asia, it targeted Australia — a market with high e-commerce growth, established loyalty habits, and only weak local cashback incumbents.</p>', 'Founded in Singapore in August 2014 by Henry Chan, Joel Leong and co-founders, ShopBack built a simple flywheel: retailers pay it a commission for referred sales, and it shares part of that commission back with shoppers as cashback. After winning across Southeast Asia, it targeted Australia — a market with high e-commerce growth, established loyalty habits, and only weak local cashback incumbents.', 1),
      ('entry-strategy', 'Raise locally to land locally', '<p>ShopBack''s November 2017 round — $32 million, backed by Australian venture investor Blue Sky — was framed to Australian media as the funding for its Australian launch. The local backer brought market knowledge, retailer introductions and domestic credibility that a purely foreign entrant would lack.</p>', 'ShopBack''s November 2017 round — \$32 million, backed by Australian venture investor Blue Sky — was framed to Australian media as the funding for its Australian launch. The local backer brought market knowledge, retailer introductions and domestic credibility that a purely foreign entrant would lack.', 2),
      ('entry-strategy', 'Lead with familiar merchants', '<p>The platform launched with mainstream Australian and global retailers shoppers already used, making cashback an overlay on existing behaviour rather than a new habit — the same low-friction model proven across its Asian markets.</p>', 'The platform launched with mainstream Australian and global retailers shoppers already used, making cashback an overlay on existing behaviour rather than a new habit — the same low-friction model proven across its Asian markets.', 3),
      ('entry-strategy', 'Ride the rails of the affiliate economy', '<p>As an affiliate-commission business, ShopBack could scale merchant coverage through networks rather than one-by-one enterprise sales, keeping entry costs low relative to loyalty-program competitors.</p>', 'As an affiliate-commission business, ShopBack could scale merchant coverage through networks rather than one-by-one enterprise sales, keeping entry costs low relative to loyalty-program competitors.', 4),
      ('entry-strategy', 'Deepen into payments', '<p>Once entrenched, ShopBack expanded beyond cashback into in-store financial services, launching ShopBack Pay in Australia alongside Singapore and Malaysia — converting a marketing utility into a payments relationship.</p>', 'Once entrenched, ShopBack expanded beyond cashback into in-store financial services, launching ShopBack Pay in Australia alongside Singapore and Malaysia — converting a marketing utility into a payments relationship.', 5),
      ('launch-and-local-iteration', NULL::text, '<ul>
...
- **Kill what doesn''t compound.** ShopBack Pay was retired in Australia in March 2026, with in-store investment redirected to the receipt-based ShopBack Receipts programme — a reminder that mature entrants keep pruning the local product line.', 6),
...
- **Regional scale advantages** — technology, merchant tooling and playbooks amortised across 13 markets.', 7),
...
- \~US\$5.5B in annual gross merchandise value', 8),
...
4. A utility that earns daily engagement can later carry higher-value services (payments) into the market.', 9)
) AS v(slug, question, html, md, ord) ON v.slug = s.slug AND s.content_id = (SELECT id FROM public.content_items WHERE slug = 'shopback-anz-market-entry' AND content_type = 'case_study');
...
FROM (VALUES
      ('Background', 'background', 1),
      ('Entry strategy', 'entry-strategy', 2),
      ('People & places', 'people-and-places', 3),
      ('Why it (mostly) worked', 'why-it-mostly-worked', 4),
      ('Key metrics', 'key-metrics', 5),
      ('Lessons', 'lessons', 6)
) AS v(title, slug, ord);
INSERT INTO public.content_bodies (content_id, section_id, question, body_text, body_markdown, sort_order, content_type)
...
- **The flagship had a story.** The Martin Place site occupies three floors of the renovated 1916 "Money Box Building" — the Commonwealth Bank landmark once depicted on children''s coin banks — surrounded by Australia''s financial institutions, giving startups a blue-chip address by the desk.', 6),
      ('why-it-mostly-worked', NULL::text, '<ul>
...
- **But**: the same growth-at-all-costs lease model that built the network nearly destroyed it globally — Australia inherited the parent''s risk without controlling it.', 7),
      ('key-metrics', NULL::text, '<ul>
...
- US\$16B parent valuation at entry; US\$800M raised in 2016 for global expansion', 8),
      ('lessons', NULL::text, '<ol>
...
4. Premium positioning survives downturns only when paired with genuinely defensible locations.', 9)
) AS v(slug, question, html, md, ord) ON v.slug = s.slug AND s.content_id = (SELECT id FROM public.content_items WHERE slug = 'wework-australia-market-entry' AND content_type = 'case_study');
